/**
 ...
 function main(params) {
    ...
    if (params.elementName === null || params.elementName === ""){
        return { "error": "Element Name is empty or undefine" };
    }
	...
	return ow.actions.invoke({ actionName: 'common-ow/digital-locker', blocking: true, result: true }).then(result => {
            ...
            var url = sysDetails.api_protocol + "://" + sysDetails.api_hostname + sysDetails.api_path + '/elements?filter[where][elementName]=' + params.elementName + '&filter[where][fk_agreementID]=' + params.agreementID;
            var options = {
                method: 'GET',
                url: url,   
                headers: {
