...
func readInActionConfigs(path string, info os.FileInfo, err error) error {
	...
	innovatorSpace := strings.Split(fullPath, "/")[1]

	actions, prs := functionsToDeploy[innovatorSpace]
	if prs == true {
		functionsToDeploy[innovatorSpace] = append(actions, a)
	} else {
		functionsToDeploy[innovatorSpace] = []*ActionFileConfig{a}
	}
	return nil
}
